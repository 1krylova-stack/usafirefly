...
<section id="banner" >
	...
	<div class="banner" style="background-image:url(<?= get_field('img',$uid); ?>);">
		<div class="container">
			...
			<div <?php if(wpm_get_language() == 'en'): //Локализация?>id="action-eng"<?php endif; ?> class="action">
				<!-- <div class="text-action"><?= get_field('action',$uid); ?></div> -->
				<div><a href="#" onclick="$(`#get_it [name='reason']`).val('Запрос прайс-листа (верхний баннер)'); return false;" data-toggle="modal" data-target="#get_it" class="y-but get-price"><?php if(wpm_get_language() == 'en'):?>GET A PRICE-LIST<?php else: ?>Получить прайс<?php endif; ?></a></div>
			</div>
		</div>
	</div>
</section>
...
<section id="link_block" >
	...
	<div class="container">
		<div class="titile"><?php if(wpm_get_language() == 'en'): //Локализация?>OUR RANGE OF PRODUCTS<?php else: ?>Полный ассортимент нашей продукции<?php endif; ?></div>		
		<div class="link_block">				
			<a class="item" href="<?php if(wpm_get_language() == 'en'): //Локализация?>/en/category/produkciya/svetovozvrashhayushhaya-furnitura-dlya-proizvoditelej-odezhdy/<?php else: ?>/category/produkciya/svetovozvrashhayushhaya-furnitura-dlya-proizvoditelej-odezhdy/<?php endif; ?>">
				...
				<span class="name"><?php if(wpm_get_language() == 'en'): //Локализация?>GARMENT ACCESORIES<?php else: ?>Световозвращающая фурнитура<br>для производителей одежды<?php endif; ?></span>
			</a>
			<a class="item" href="<?php if(wpm_get_language() == 'en'): //Локализация?>/en/category/produkciya/katalog-svetovozvrashhayushhej-produkcii/<?php else: ?>/category/produkciya/katalog-svetovozvrashhayushhej-produkcii/<?php endif; ?>">
				<span class="image webpcheck" style="">
				</span>
				<span class="name"><?php if(wpm_get_language() == 'en'): //Локализация?>CATALOGUE OF OUR REFLECTIVE ACCESORIES<?php else: ?>Каталог<br>световозвращающей продукции<?php endif; ?></span>
			</a>
		</div>
	</div>
					...
					<div class="left-60">
						<div class="contact-form">
							<form method="POST" action="/mail.php" data-with-ajax="">
								...
								<div class="ssf"><div class="left"><label><?php if(wpm_get_language() == 'en'): //Локализация?>QUESTION<?php else: ?>ВОПРОС<?php endif; ?>:</label></div><div class="right"><textarea name="question"></textarea></div><div class="clearfix"></div></div>
